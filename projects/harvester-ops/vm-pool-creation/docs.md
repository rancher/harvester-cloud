...
| <a name="input_nfs_data_disk_size"></a> [nfs\_data\_disk\_size](#input\_nfs\_data\_disk\_size) | Specifies the size of the data disk used for NFS exports, in GB. Only used when 'nfs\_server\_install' is true. Default is '50'. | `number` | `100` | no |
| <a name="input_nfs_export_options"></a> [nfs\_export\_options](#input\_nfs\_export\_options) | Specifies the NFS export options for /etc/exports. Only used when 'nfs\_server\_install' is true. Default is '*(rw,sync,no\_subtree\_check,no\_root\_squash)'. | `string` | `"*(rw,sync,no_subtree_check,no_root_squash)"` | no |
| <a name="input_nfs_export_path"></a> [nfs\_export\_path](#input\_nfs\_export\_path) | Specifies the directory path to export via NFS. Only used when 'nfs\_server\_install' is true. Default is '/mnt/nfs-data'. | `string` | `"/mnt/nfs-data"` | no |
| <a name="input_nfs_server_install"></a> [nfs\_server\_install](#input\_nfs\_server\_install) | Enables the automated installation of an NFS server on the VM during startup. Default is 'false'. | `bool` | `false` | no |
