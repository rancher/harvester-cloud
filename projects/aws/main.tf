...
locals {
  ...
  instance_type = (
    local.harvester_node_count == 1 ? (local.harvester_cluster_size == "small" ? "m8i.4xlarge" : "m8i.8xlarge") :
    local.harvester_node_count == 3 ? (local.harvester_cluster_size == "small" ? "m8i.8xlarge" : "m8i.16xlarge") :
    "m8i.16xlarge"
  )
...
resource "null_resource" "harvester_iso_download_checking" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      host        = module.harvester_node.instances_public_ip
...
resource "ssh_resource" "retrieve_kubeconfig" {
  ...
  host       = module.harvester_node.instances_public_ip
  commands = [
    "sudo sed 's/127.0.0.1/${module.harvester_node.instances_public_ip}/g' /tmp/rke2.yaml"
  ]
  user        = local.ssh_username
  private_key = data.local_file.ssh_private_key.content
}
